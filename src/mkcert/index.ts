...
class Mkcert {
  ...
  public static create(options: MkcertBaseOptions) {
    return new Mkcert(options);
  }

  private constructor(options: MkcertBaseOptions) {
    const {
      force,
      autoUpgrade,
      source,
      mkcertPath,
      savePath = SSLFY_DATA_DIR,
      keyFileName = "dev.pem",
  ...
  public async renew(hosts: string[]) {
    ...
    console.debug(
      "Neither hosts nor hash has changed, skip regenerate certificate"
    );
  }
